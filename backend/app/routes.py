...
def face_verify():
    ...
    try:
        result = verify_face(session["username"], data["image"])
        if result:
            db.session.add(
                AuditLog(user_id=session["user_id"], action="face_login", ip_address=request.remote_addr, success=True)
            )
            db.session.commit()
            return jsonify({"verified": True}), 200
        return jsonify({"verified": False, "error": "Face does not match"}), 401
    except Exception as e:
